...
class AIExtremeMoveHunter(BaseStrategy):
    def evaluate(self,m):
        ...
        confidence=float(z.get("confidence") or 0)
        if confidence<float(p.get("ai_confidence_floor",.14)):return None
        x=m.df("15m").iloc[-1];atr=safe(x.atr14);side=z["side"];sg=1 if side=="long" else -1
        stop=m.price-sg*p["atr_stop"]*atr;score=.58+.36*confidence
        reason=f"AI extreme-tail {side.upper()} · predicted L={z['long_pred']:.4f} S={z['short_pred']:.4f} · adaptive tail threshold"
        return sig(self.spec,m,side,score,stop,reason,"15m",{"ai_tail_long":z["long_pred"],"ai_tail_short":z["short_pred"],"ai_confidence":confidence,"ai_long_threshold":z["long_threshold"],"ai_short_threshold":z["short_threshold"],"ai_model_n":min(z["long_n"],z["short_n"])})
...
SPECS=[
 ...
 StrategySpec("ai_extreme_hunter","AI 十倍極端行情獵手","Online tail model learns precursor patterns from fully closed 15m bars; long 10x / short crash objective",P(ai_confidence_floor=.14,atr_stop=1.55,risk_pct=.014,max_positions=3,sl1_r=.68,sl1_fraction=.16,tp1_r=.9,tp2_r=2.2,tp3_r=5.0,tp1_fraction=.20,tp2_fraction=.25,breakeven_trigger_r=1.0,breakeven_offset_r=.06,trail_start_r=1.8,trail_r=1.40),"15m",("15m","1H","4H"),.56,.64,.70),
]
