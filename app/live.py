...
import asyncio,base64,hashlib,hmac,json,math,time,uuid
...
class BitgetLiveAdapter:
    ...
    async def allocate(self,position,params,price,account,live_positions):
        ...
        if notional<=0:raise PermissionError("shared portfolio allocator returned zero; total/free-margin cap reached")
    ...
    async def _wait_position(self,symbol,side,tries=8):
    ...
    def _plan_trigger(self,p,role):
        keys=["stopLossTriggerPrice","triggerPrice"] if role=="STOP" else ["stopSurplusTriggerPrice","triggerPrice"]
        for k in keys:
            try:
    ...
    async def _place_partial_tp(self,order,role,trigger,size):
        oid=f"{order['client_oid']}-{role.lower()}";p={"marginCoin":settings.bitget_margin_coin,"productType":settings.bitget_product_type,"symbol":order["symbol"],"planType":"profit_plan","triggerPrice":str(trigger),"triggerType":"mark_price","executePrice":"0","holdSide":self._hold_side(order["side"]),"size":str(size),"clientOid":oid};return oid,await self._private("POST","/api/v2/mix/order/place-tpsl-order",payload=p)
    async def _cancel_plan(self,order,prot):
        if not (prot.get("order_id") or prot.get("client_oid")):return
        try:await self._private("POST","/api/v2/mix/order/cancel-plan-order",payload={"orderIdList":[{"orderId":prot.get("order_id") or "","clientOid":prot.get("client_oid") or ""}],"symbol":order["symbol"],"productType":settings.bitget_product_type,"marginCoin":settings.bitget_margin_coin})
        except Exception:pass
    async def ensure_protections(self,order,paper_pos=None):
        detail=json.loads(order.get("detail_json") or "{}");plan=detail.get("plan") or {};stop=float((paper_pos or {}).get("stop") or plan.get("stop") or 0);tp1=float((paper_pos or {}).get("tp1") or plan.get("tp1") or 0);tp2=float((paper_pos or {}).get("tp2") or plan.get("tp2") or 0);tp3=float((paper_pos or {}).get("tp3") or plan.get("tp3") or 0);f1=float((paper_pos or {}).get("tp1_fraction") or plan.get("tp1_fraction") or .3);f2=float((paper_pos or {}).get("tp2_fraction") or plan.get("tp2_fraction") or .35)
        if stop<=0 or tp3<=0:raise RuntimeError("missing protection plan prices")
        contract=(await self.contracts()).get(order["symbol"],{});places=int(contract.get("volumePlace") or 6);step=contract.get("sizeMultiplier") or str(10**-places);total=Decimal(str(order["qty"]));q1=self._floor_step(float(total)*f1,step,places);q2=self._floor_step(float(total)*f2,step,places)
        required=[("STOP",stop,Decimal("0")),("TP3",tp3,Decimal("0"))]
        if settings.live_partial_tp_enabled:required=[("STOP",stop,Decimal("0")),("TP1",tp1,q1),("TP2",tp2,q2),("TP3",tp3,Decimal("0"))]
        for attempt in range(max(1,settings.live_protection_retry)):
            ...
            for role,trig,size in required:
                ...
                if not found:
                    # Only adopt plans carrying our own clientOid. Entry presets are an immediate fallback,
                    # but the Guardian always creates dedicated modifiable protection plans for long-term control.
                    for x in pending:
                ...
                else:missing.append((role,trig,size))
            if not missing:return True
            for role,trig,size in missing:
                try:
                    if role in {"STOP","TP3"}:
                        ...
                        cid,res=await self._place_partial_tp(order,role,trig,size);oid=(res or {}).get("orderId") if isinstance(res,dict) else "";self._upsert_protection(order["id"],role,str(oid or ""),cid,trig,float(size),"LIVE",res)
        ...
        verified={x["role"] for x in db.query("SELECT role FROM live_protections WHERE live_order_id=? AND status='LIVE'",(order["id"],))}
        critical_ok="STOP" in verified and bool(verified & {"TP1","TP2","TP3"})
        if critical_ok:
            db.risk_event("LIVE_PROTECTION_PARTIAL","Critical STOP + at least one TP are verified; missing partial targets will keep being repaired",order["strategy"],"champion",order["symbol"])
            return True
        db.risk_event("LIVE_PROTECTION_INCOMPLETE","Protection Guardian could not verify a critical STOP and at least one TP plan",order["strategy"],"champion",order["symbol"])
    ...
    async def sync_from_paper_for_symbol(self,symbol):
        ...
        for order in db.query("SELECT * FROM live_orders WHERE symbol=? AND status='OPEN'",(symbol,)):
            ...
            if not prot:
                await self.ensure_protections(order,pos);continue
            ...
            try:res=await self._private("POST","/api/v2/mix/order/modify-tpsl-order",payload=payload);self._upsert_protection(order["id"],"STOP",str((res or {}).get("orderId") or prot.get("order_id") or ""),str((res or {}).get("clientOid") or prot.get("client_oid") or ""),desired,0,"LIVE",res)
                ...
                db.risk_event("LIVE_STOP_MODIFY_FAILED",str(e),order["strategy"],"champion",symbol);db.execute("UPDATE live_protections SET status='STALE' WHERE id=?",(prot["id"],));await self.ensure_protections(order,pos)
    ...
    async def place_from_paper(self,position,params):
        ...
        for lp in live_positions:
            if str(lp.get("symbol") or "").upper()==position["symbol"].upper() and self._position_size(lp)>0:raise PermissionError("existing Bitget position on this symbol; no stacking with manual/external position")
        ...
        acct=await self.account(position["symbol"]);alloc=await self.allocate(position,params,price,acct,live_positions);qty,lev,contract=await self.normalize(position["symbol"],alloc["notional"]/price,price,alloc["leverage"]);actual=float(qty)*price
